...
// src/text-editor.js
var aslWriteStream = StreamLanguage.define({
  startState() {
    return {
      mode: "asl",
...
var aslWriteTheme = EditorView.theme({
  "&": {
    height: "100%",
    fontSize: "0.95rem",
...
var aslWriteHighlight = HighlightStyle.define([
...
var errorDecoField = StateField.define({
  create() {
    return Decoration.none;
  },
  update(deco, tr) {
...
var setErrorDeco = StateEffect.define();
...
function peg$parse(input, options) {
  ...
  function peg$f4(idens, graphemes) {
    return node("Definition", { idens, graphemes });
  }
  function peg$f5(idens, grapheme) {
    return node("Definition", { idens, graphemes: [grapheme] });
  }
  function peg$f6(head, tail) {
    return flatten2(head, tail, 2);
  }
  function peg$f7(head, tail) {
    return flatten2(head, tail);
  }
  function peg$f8(comms) {
  ...
  function peg$f11(comms) {
    return node("Bracket", { comms });
  }
  function peg$f12(name2, args) {
    return node("Command", { name: name2, args });
  }
  function peg$f13(name2) {
    return node("Command", { name: name2, args: [] });
  }
  function peg$f14(head, tail) {
    return flatten2(head, tail);
  }
  function peg$f15(ce2) {
    return ce2;
  }
  ...
  function peg$f20(op, child) {
    return node("UnaryOp", { op, child });
  }
  function peg$f21(segments) {
    return node("Body", { segments });
  }
  function peg$f22(graphemes) {
    return node("AslInline", { graphemes });
  }
  function peg$f23(rows) {
  ...
  function peg$f31(text2) {
    return "```" + text2 + "```";
  }
  function peg$f32(text2) {
    return "`" + text2 + "`";
  }
  function peg$f33(text2) {
    return "$$" + text2 + "$$";
  }
  function peg$f34(text2) {
    return "$" + text2 + "$";
  }
  function peg$f35(name2) {
...
var Env = class {
  ...
  lookup(name2) {
    if (this._data.has(name2)) {
      return this._data.get(name2);
    }
    throw new Error(`No variable called '${name2}' has been defined.`);
  }
  define(name2, val) {
    this._data.set(name2, val);
  }
};
  ...
  toSVG() {
    ...
    if (this.debug) {
      ...
      for (const cp of this.debugControlPoints) {
        debugElems += `<circle cx="${cp.c1x}" cy="${cp.c1y}" r="4" fill="blue" opacity="0.6"/><circle cx="${cp.c2x}" cy="${cp.c2y}" r="4" fill="green" opacity="0.6"/><line x1="${cp.fromX}" y1="${cp.fromY}" x2="${cp.c1x}" y2="${cp.c1y}" stroke="blue" stroke-width="4" stroke-dasharray="3,3" opacity="0.6"/><line x1="${cp.x}" y1="${cp.y}" x2="${cp.c2x}" y2="${cp.c2y}" stroke="green" stroke-width="4" stroke-dasharray="3,3" opacity="0.6"/>`;
      }
    }
...
var ie3 = k(Q).replace("hr", A).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("|table", "").replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)])[ \\t]").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", q).getRegex();
var Me = k(/^( {0,3}> ?(paragraph|[^\n]*)(?:\n|$))+/).replace("paragraph", ie3).getRegex();
var U = { blockquote: Me, code: Oe, def: $e, fences: we, heading: ye, hr: A, html: Le, lheading: se, list: _e, newline: Te, paragraph: ie3, table: _, text: Se };
...
var C2 = { normal: U, gfm: ze, pedantic: Ee };
...
var Visitor = class {
  ...
  visitIden(node2) {
    const val = this.env.lookup(node2.name);
    return val;
  }
  ...
  visitCommand(node2) {
    ...
    this.turtle.runCommand(node2.name, args);
  }
  ...
  flattenGraphemes(result) {
    if (result?.type === "Graphemes") {
      return result.val.flatMap((v2) => this.flattenGraphemes(v2));
    }
    return [result];
  }
  // = "@@@" _ rows:BlockRows _ "@@@"
  ...
  visitAslInline(node2) {
    const glyphs = node2.graphemes.flatMap((g2) => this.visitGrapheme(g2));
...
var tabContents = {
  ...
  vocab: vocab_default,
  diacritics: diacritics_default,
  handshapes: handshapes_default,
...
async function resolveImports(ast) {
...
async function execute() {
  ...
  try {
    ...
    if (e.location) {
      ...
      out2.srcdoc = `<!DOCTYPE html><html><body><div style="color:#911c1c;font-family:monospace;padding:12px;">${e.message}</div></body></html>`;
    }
  }
}
