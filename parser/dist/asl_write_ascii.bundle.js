      ...
      */
      wordAt(pos) {
        ...
        while (start > 0) {
          ...
          if (cat2(text.slice(prev, start)) != CharCategory.Word)
            break;
          start = prev;
        }
        while (end < length) {
...
function scrollableParents(dom, getX = true) {
  ...
  for (let cur = dom.parentNode; cur; ) {
    if (cur == doc2.body || (!getX || x2) && y2) {
      break;
    } else if (cur.nodeType == 1) {
      if (!y2 && cur.scrollHeight > cur.clientHeight)
        y2 = cur;
      if (getX && !x2 && cur.scrollWidth > cur.clientWidth)
        x2 = cur;
      cur = cur.assignedSlot || cur.parentNode;
    } else if (cur.nodeType == 11) {
      cur = cur.host;
...
function posAtCoords(view, coords, precise, scanY) {
  ...
  return new InlineCoordsScan(view, x2, y2, view.textDirectionAt(block.from)).scanTile(line, block.from);
}
  ...
  "node_modules/@codemirror/view/dist/index.js"() {
          ...
          span: (from2, to2, active, openStart) => {
            for (let pos = from2; pos < to2; ) {
              ...
              if (chars == null) {
                ...
                b2.ensureLine(pendingLineAttrs);
                b2.addText(chars, active, pos == from2 ? openStart : active.length);
                pos += chars.length;
              }
              pendingLineAttrs = null;
            }
    ...
    HeightMap = class _HeightMap {
      ...
      static of(nodes) {
        ...
        return new HeightMapBranch(_HeightMap.of(nodes.slice(0, i)), brk, _HeightMap.of(nodes.slice(j2)));
      }
    };
...
function eqSelectionShape(a, b2) {
  return a.ranges.length == b2.ranges.length && a.ranges.filter((r, i) => r.empty != b2.ranges[i].empty).length === 0;
}
function conc(a, b2) {
...
function highlightError(view, start, end) {
...
function clearErrors(view) {
  view.dispatch({ effects: setErrorDeco.of(Decoration.none) });
}
function getEditorContent(view) {
  return view.state.doc.toString();
}
var aslWriteStream, aslWriteTheme, aslWriteHighlight, errorDecoField, setErrorDeco;
var init_text_editor = __esm({
  "src/text-editor.js"() {
    ...
    init_dist5();
    init_dist4();
    aslWriteStream = StreamLanguage.define({
      startState() {
        return {
          mode: "asl",
          // 'asl' | 'markdown' | 'aslInline' | 'aslBlock' | 'blockComment' | 'markdownCode' | 'markdownFenced' | 'markdownMath' | 'markdownDisplayMath'
          seenSeparator: false
        };
      },
      token(stream, state) {
        ...
        if (state.mode === "markdown") {
          if (stream.match("```")) {
            state.mode = "markdownFenced";
            return "comment";
          }
          if (stream.match("`")) {
            state.mode = "markdownCode";
            return "comment";
          }
          if (stream.match("$$")) {
            state.mode = "markdownDisplayMath";
            return "comment";
          }
          if (stream.match("$")) {
            state.mode = "markdownMath";
            return "comment";
          }
          if (stream.match("@@@")) {
            state.mode = "aslBlock";
    ...
    aslWriteTheme = EditorView.theme({
      "&": {
        height: "100%",
        fontSize: "0.95rem",
        ...
        //TODO: why aren't these working in the editor?
      },
      ".cm-error": {
        display: "inline-block",
    ...
    aslWriteHighlight = HighlightStyle.define([
    ...
    errorDecoField = StateField.define({
      create() {
        return Decoration.none;
      },
      update(deco, tr) {
    ...
    setErrorDeco = StateEffect.define();
  }
});
...
function flatten2(head, tail, index = 1) {
  return flattenMap(head, tail, (t2) => t2[index]);
}
...
function peg$parse(input, options) {
  ...
  const peg$r3 = /^[^\n\r=]/;
  const peg$r4 = /^[^\n\r]/;
  const peg$r5 = /^[_a-zA-Z~]/;
  const peg$r6 = /^[_a-zA-Z0-9~]/;
  const peg$r7 = /^[0-9]/;
  ...
  const peg$e25 = peg$classExpectation(["_", ["a", "z"], ["A", "Z"], "~"], false, false, false);
  const peg$e26 = peg$classExpectation(["_", ["a", "z"], ["A", "Z"], ["0", "9"], "~"], false, false, false);
  ...
  function peg$f0(doc2) {
    return doc2;
  }
  function peg$f1(statements, body) {
    return node("Document", { statements, body });
  }
  function peg$f2(head, tail) {
    return flatten2(head, tail);
  }
  function peg$f3(path) {
    return node("Import", { path: path.trim() });
  }
  function peg$f4(idens, graphemes) {
    return node("GraphemeDef", { idens, args: null, graphemes });
  }
  function peg$f5(idens, grapheme) {
    return node("GraphemeDef", { idens, args: null, graphemes: [grapheme] });
  }
  function peg$f6(idens, args, graphemes) {
    return node("GraphemeDef", { idens, args, graphemes });
  }
  function peg$f7(idens, args, grapheme) {
    return node("GraphemeDef", { idens, args, graphemes: [grapheme] });
  }
  function peg$f8(idens, expr) {
    return node("ExprDef", { idens, args: null, expr });
  }
  function peg$f9(idens, args, expr) {
    return node("ExprDef", { idens, args, expr });
  }
  ...
  function peg$f12(fname, args) {
    return node("Call", { fname, args });
  }
  function peg$f13(comms) {
    return node("Block", { comms });
  }
  ...
  function peg$f16(comms) {
    return node("Bracket", { comms });
  }
  function peg$f17(name2, args) {
    return node("Command", { name: name2, args });
  }
  function peg$f18(name2) {
    return node("Command", { name: name2, args: [] });
  }
  ...
  function peg$f21(ce2) {
    return ce2;
  }
  function peg$f22(head, tail) {
    if (tail.length === 0) return head;
    return node("Sum", { head, tail: tail.map((t2) => ({ op: t2[1], val: t2[3] })) });
  }
  function peg$f23(head, tail) {
    if (tail.length === 0) return head;
    return node("Prod", { head, tail: tail.map((t2) => ({ op: t2[1], val: t2[3] })) });
  }
  function peg$f24(expr) {
    return expr;
  }
  function peg$f25(expr) {
    return expr;
  }
  function peg$f26(op, child) {
    return node("UnaryOp", { op, child });
  }
  function peg$f27(fname, args) {
    return node("Call", { fname, args });
  }
  function peg$f28(segments) {
    return node("Body", { segments });
  }
  function peg$f29(graphemes) {
    return node("AslInline", { graphemes });
  }
  function peg$f30(rows) {
  ...
  function peg$f34(chars) {
    return node("MarkdownText", { text: chars.join("") });
  }
  function peg$f35() {
    return "\\\\";
  }
  ...
  function peg$f37(char) {
    return char;
  }
  function peg$f38(text2) {
    return "```" + text2 + "```";
  }
  function peg$f39(text2) {
    return "`" + text2 + "`";
  }
  function peg$f40(text2) {
    return "$$" + text2 + "$$";
  }
  function peg$f41(text2) {
    return "$" + text2 + "$";
  }
  function peg$f42(name2) {
    return node("Iden", { name: name2 });
  }
  ...
  function peg$parseDefinition() {
    let s0;
    s0 = peg$parseGraphemeDef();
    if (s0 === peg$FAILED) {
      s0 = peg$parseExprDef();
    }
    return s0;
  }
  function peg$parseGraphemeDef() {
    let s0, s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, s12, s13;
    s0 = peg$currPos;
    s1 = peg$parseIdenList();
    if (s1 !== peg$FAILED) {
      s2 = peg$parse_();
  ...
  function peg$parseGraphemes() {
    let s0, s1, s2, s3, s4, s5;
    s0 = peg$currPos;
    s1 = peg$parseGrapheme();
    if (s1 !== peg$FAILED) {
      s2 = [];
      s3 = peg$currPos;
      s4 = peg$parse_();
  ...
  function peg$parseArgListOneLine() {
    let s0, s1, s2, s3, s4, s5;
    s0 = peg$currPos;
    s1 = peg$parseArg();
    if (s1 !== peg$FAILED) {
  ...
  function peg$parseTerminalExpr() {
    let s0, s1, s2, s3, s4, s5, s6, s7;
    s0 = peg$currPos;
  ...
  function peg$parseBodySegment() {
    let s0;
    s0 = peg$parseAslBlock();
    if (s0 === peg$FAILED) {
      s0 = peg$parseAslInline();
      if (s0 === peg$FAILED) {
        s0 = peg$parseMarkdownText();
      }
    }
    return s0;
  }
  function peg$parseAslInline() {
    let s0, s1, s2, s3, s4, s5;
    s0 = peg$currPos;
  ...
  function peg$parseBlockRows() {
    let s0, s1, s2, s3, s4, s5;
    s0 = peg$currPos;
    s1 = peg$parseBlockRow();
    if (s1 !== peg$FAILED) {
      s2 = [];
      s3 = peg$currPos;
      s4 = peg$parseBlockRowSep();
      if (s4 !== peg$FAILED) {
        s5 = peg$parseBlockRow();
        if (s5 !== peg$FAILED) {
          s4 = [s4, s5];
          s3 = s4;
        } else {
  ...
  function peg$parseBlockRow() {
    ...
    s2 = peg$parseGrapheme();
    if (s2 !== peg$FAILED) {
      s3 = [];
      s4 = peg$currPos;
      s5 = peg$parseSPACE();
      if (s5 !== peg$FAILED) {
        s6 = peg$parseGrapheme();
        if (s6 !== peg$FAILED) {
          s5 = [s5, s6];
          s4 = s5;
        } else {
  ...
  function peg$parseMarkdownText() {
    let s0, s1, s2;
    s0 = peg$currPos;
    s1 = [];
  ...
  "src/parser.js"() {
    peg$SyntaxError = class extends SyntaxError {
      ...
      format(sources) {
        ...
        if (this.location) {
          let src = null;
          const st2 = sources.find((s2) => s2.source === this.location.source);
          if (st2) {
            src = st2.text.split(/\r\n|\n|\r/g);
          }
...
var init_env = __esm({
  "src/env.js"() {
    Env = class {
      ...
      lookup(name2, type) {
        if (type === void 0) throw new Error("Must specify type when looking up variable");
        const key = cat(name2, type);
        console.log("[LOOKUP]", `${key} ->`);
        if (this._data.has(key)) {
          console.log("[LOOKUP]", this._data.get(key));
          return this._data.get(key);
        }
        const thunkKey = `thunk:${name2}`;
        if (this._data.has(thunkKey)) {
          return this._data.get(thunkKey).as(type);
        }
        if (this._parent) {
          return this._parent.lookup(name2, type);
        }
        throw new Error(`No variable of type '${type}' called '${name2}' has been defined.`);
      }
      define(name2, val) {
        const key = cat(name2, val.type);
...
var init_matrix = __esm({
  "src/matrix.js"() {
    Mat2D = class _Mat2D {
      constructor(a = 1, b2 = 0, c = 0, d = 1, e = 0, f = 0) {
        this.a = a;
        this.b = b2;
        this.c = c;
        this.d = d;
      ...
      static translation(x2, y2) {
        return new _Mat2D(1, 0, 0, 1, x2, y2);
      }
      static rotation(rad) {
        const c = Math.cos(rad);
        const s = Math.sin(rad);
        return new _Mat2D(c, -s, s, c, 0, 0);
      }
      ...
      transformPoint(x2, y2) {
        return {
          x: this.a * x2 + this.c * y2 + this.e,
          y: this.b * x2 + this.d * y2 + this.f
        };
      }
      toSVG() {
...
// src/turtle.js
var signatures, SVGSeg, SVGPath, Turtle;
var init_turtle = __esm({
  "src/turtle.js"() {
    ...
    signatures = {
      ...
      goto: [{
        types: ["Number", "Number"],
        method: (turtle, x2, y2) => turtle.goto(x2, y2)
      }],
      curveto: [{
        types: ["Number", "Number", "Number", "Number", "Number", "Number"],
        method: (turtle, x2, y2, c1x, c1y, c2x, c2y) => turtle.curveto(x2, y2, c1x, c1y, c2x, c2y)
      }, {
        types: ["Number", "Number", "Number", "Number"],
        method: (turtle, x2, y2, cx, cy) => turtle.curveto(x2, y2, cx, cy, cx, cy)
      }],
      smoothto: [{
        types: ["Number", "Number", "Number", "Number"],
        method: (turtle, x2, y2, cx, cy) => turtle.smoothto(x2, y2, cx, cy, cx, cy)
      }],
      arcto: [{
        types: ["Number", "Number", "Number"],
        method: (turtle, x2, y2, r) => turtle.arcto(x2, y2, r, r, false, 1)
      }, {
        types: ["Number", "Number", "Number", "Number"],
        method: (turtle, x2, y2, r, dir) => turtle.arcto(x2, y2, r, r, false, dir)
      }, {
        types: ["Number", "Number", "Number", "Number", "Number"],
        method: (turtle, x2, y2, rx, ry, dir) => turtle.arcto(x2, y2, rx, ry, false, dir)
      }],
      largearcto: [{
        types: ["Number", "Number", "Number"],
        method: (turtle, x2, y2, r) => turtle.arcto(x2, y2, r, r, true, 1)
      }, {
        types: ["Number", "Number", "Number", "Number"],
        method: (turtle, x2, y2, r, dir) => turtle.arcto(x2, y2, r, r, true, dir)
      }, {
        types: ["Number", "Number", "Number", "Number", "Number"],
        method: (turtle, x2, y2, rx, ry, dir) => turtle.arcto(x2, y2, rx, ry, true, dir)
      }],
      draw: [{
        types: ["Graphemes"],
        method: (turtle, gs) => gs.forEach((g2) => turtle.draw(g2))
      }, {
        types: ["Graphemes", "Number", "Number"],
        method: (turtle, gs, w2, h) => gs.forEach((g2) => turtle.draw(g2, w2, h))
      }],
      penup: [{
        types: [],
      ...
      flip: [{
        ...
        method: (turtle, gs) => {
          ...
          turtle.flip();
          gs.forEach((g2) => turtle.draw(g2));
        }
      }, {
        types: ["Graphemes", "Number", "Number"],
        method: (turtle, gs, w2, h) => {
          turtle.flip();
          gs.forEach((g2) => turtle.draw(g2, w2, h));
        }
      }],
      pendown: [{
        types: [],
    ...
    Turtle = class {
      ...
      penDown() {
        ...
        this.isPenDown = true;
        this.currentPath.push(new SVGSeg("M", this.transform.e, this.transform.f));
      }
      penUp() {
        if (!this.isPenDown) return;
        this.isPenDown = false;
      ...
      draw(grapheme, w2 = null, h = null) {
        console.log("[DRAW]", this);
        let transformMatrix = this.transform;
        if (w2 !== null && h !== null) {
          const scaleX = w2 / grapheme.turtle.width;
      ...
      forward(dist2) {
        this.transform = this.transform.multiply(Mat2D.translation(dist2, 0));
        const x2 = this.transform.e;
        const y2 = this.transform.f;
        if (this.isPenDown) {
          this.currentPath.push(new SVGSeg("L", x2, y2));
        }
      }
      goto(x2, y2) {
        console.log("[GOTO]", x2, y2);
        if (this.isPenDown) {
          this.currentPath.push(new SVGSeg("L", x2, y2));
        }
        this.transform.e = x2;
      ...
      curveto(x2, y2, c1x, c1y, c2x, c2y) {
        if (this.isPenDown) {
          this.currentPath.push(new SVGSeg("C", c1x, c1y, c2x, c2y, x2, y2));
        }
        if (this.debug) {
          this.debugControlPoints.push({ x: x2, y: y2, c1x, c1y, c2x, c2y, fromX: this.transform.e, fromY: this.transform.f });
        }
        this.transform.e = x2;
        this.transform.f = y2;
      }
      smoothto(x2, y2, cx, cy) {
        if (this.isPenDown) {
          this.currentPath.push(new SVGSeg("S", cx, cy, x2, y2));
        }
        if (this.debug) {
          this.debugControlPoints.push({ x: x2, y: y2, c2x: cx, c2y: cy, fromX: this.transform.e, fromY: this.transform.f });
        }
        this.transform.e = x2;
        this.transform.f = y2;
      }
      ...
      arcto(x2, y2, rx, ry, large, dir) {
        const sweep = dir >= 0 ? 1 : 0;
        const largeFlag = large ? 1 : 0;
        if (this.isPenDown) {
          this.currentPath.push(new SVGSeg("A", Math.abs(rx), Math.abs(ry), 0, largeFlag, sweep, x2, y2));
        }
        if (this.debug) {
      ...
      toSVG() {
        ...
        if (this.debug) {
          ...
          debugElems = `<circle cx="${x2}" cy="${y2}" r="4" fill="red" opacity="0.7"/><line x1="${x2}" y1="${y2}" x2="${x2 + dx}" y2="${y2 + dy}" stroke="red" stroke-width="4" opacity="0.6" marker-end="url(#debug-arrow)"/>`;
          for (const cp of this.debugControlPoints) {
            if (cp.c1x != void 0) {
              debugElems += `<circle cx="${cp.c1x}" cy="${cp.c1y}" r="4" fill="blue" opacity="0.6"/><line x1="${cp.fromX}" y1="${cp.fromY}" x2="${cp.c1x}" y2="${cp.c1y}" stroke="blue" stroke-width="4" stroke-dasharray="3,3" opacity="0.6"/>`;
            }
            debugElems += `<circle cx="${cp.c2x}" cy="${cp.c2y}" r="4" fill="green" opacity="0.6"/><line x1="${cp.x}" y1="${cp.y}" x2="${cp.c2x}" y2="${cp.c2y}" stroke="green" stroke-width="4" stroke-dasharray="3,3" opacity="0.6"/>`;
          }
          for (const arc of this.debugArcs) {
            debugElems += `<circle cx="${arc.cx}" cy="${arc.cy}" r="4" fill="purple" opacity="0.6"/><line x1="${arc.cx}" y1="${arc.cy}" x2="${arc.x1}" y2="${arc.y1}" stroke="purple" stroke-width="4" stroke-dasharray="3,3" opacity="0.6"/><line x1="${arc.cx}" y1="${arc.cy}" x2="${arc.x2}" y2="${arc.y2}" stroke="purple" stroke-width="4" stroke-dasharray="3,3" opacity="0.6"/>`;
          }
        }
        const viewBox = `${-this.width / 2} ${-this.height / 2} ${this.width} ${this.height}`;
        const fullHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${viewBox}" width="3rem" height="3rem" overflow="visible"><defs><marker id="debug-arrow" viewBox="0 0 10 10" refX="8" refY="5"markerWidth="4" markerHeight="4" orient="auto-start-reverse"><path d="M2 1L8 5L2 9" fill="none" stroke="red" stroke-width="2"/></marker></defs><g transform="scale(1,-1)">${innerPaths}${debugElems}</g></svg>`;
        return { type: "Grapheme", turtle: this, fullHTML, innerPaths: innerPaths + debugElems };
      }
      runCommand(name2, args, unwrapArg, flattenGraphemes) {
        if (!signatures.hasOwnProperty(name2)) {
          throw new Error(`No drawing command called: ${name2}`);
        }
        const overloads = signatures[name2];
        const arityMatches = overloads.filter((sig) => sig.types.length === args.length);
        if (arityMatches.length === 0) {
          const possible = [...new Set(overloads.map((s2) => s2.types.length))];
          const s = possible.length > 1 ? "s" : "";
          throw new Error(`${name2} takes ${possible.join(" or ")} argument${s}, not ${args.length}.`);
        }
        for (const sig of arityMatches) {
          ...
          if (mismatchIndex === -1) {
            ...
            throw new Error(
              `Argument ${mismatchIndex + 1} of ${name2} must be type '${expected}', not '${actual}'.`
            );
          }
        }
      }
...
function k(u3, e = "") {
  let t2 = typeof u3 == "string" ? u3 : u3.source, n = { replace: (r, i) => {
...
function J(u3, e) {
  let t2 = u3.replace(m.findPipe, (i, s, a) => {
    let o = false, l = s;
    for (; --l >= 0 && a[l] === "\\"; ) o = !o;
    return o ? "|" : " |";
...
function me(u3, e, t2, n, r) {
  let i = e.href, s = e.title || null, a = u3[1].replace(r.other.outputLinkReplace, "$1");
  n.state.inLink = true;
  let o = { type: u3[0].charAt(0) === "!" ? "image" : "link", raw: t2, href: i, title: s, text: a, tokens: n.inlineTokens(a) };
...
var T, _, Re, m, Te, Oe, we, A, ye, N, re, se, Pe, Q, Se, j, $e, _e, q, F, Le, ie3, Me, U, te, ze, Ee, Ie, Ae, oe, Ce, v, K, ae, Be, le, De, qe, ue, ve, He, Ge, pe, Ze, Ne, ce, Qe, je, Fe, Ue, Ke, We, Xe, Je, Ve, Ye, D, et, he, ke, tt, ne, W, nt, Z, rt, C2, z, st, de, w, x, y, $, b, P, B, L, Ut, Kt, Wt, Xt, Jt, Yt, en;
var init_marked_esm = __esm({
  "node_modules/marked/lib/marked.esm.js"() {
    ...
    ie3 = k(Q).replace("hr", A).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("|table", "").replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)])[ \\t]").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", q).getRegex();
    Me = k(/^( {0,3}> ?(paragraph|[^\n]*)(?:\n|$))+/).replace("paragraph", ie3).getRegex();
    U = { blockquote: Me, code: Oe, def: $e, fences: we, heading: ye, hr: A, html: Le, lheading: se, list: _e, newline: Te, paragraph: ie3, table: _, text: Se };
    ...
    C2 = { normal: U, gfm: ze, pedantic: Ee };
    ...
    w = class {
      ...
      code(e) {
        ...
        if (t2) {
          ...
          return { type: "code", raw: t2[0], codeBlockStyle: "indented", text: this.options.pedantic ? n : E(n, `
`) };
        }
      }
      fences(e) {
...
` + n.join(`
`), S = this.blockquote(f);
              s[s.length - 1] = S, r = r.substring(0, r.length - R2.raw.length) + S.raw, i = i.substring(0, i.length - R2.text.length) + S.text;
              break;
            } else if (h?.type === "list") {
              let R2 = h, f = R2.raw + `
` + n.join(`
`), S = this.list(f);
              s[s.length - 1] = S, r = r.substring(0, r.length - h.raw.length) + S.raw, i = i.substring(0, i.length - R2.raw.length) + S.raw, n = f.substring(s.at(-1).raw.length).split(`
`);
              continue;
            }
          }
...
`, 1)[0], R2 = !d.trim(), f = 0;
            if (this.options.pedantic ? (f = 2, c = d.trimStart()) : R2 ? f = t2[1].length + 1 : (f = d.search(this.rules.other.nonSpaceChar), f = f > 4 ? 1 : f, c = d.slice(f), f += t2[1].length), R2 && this.rules.other.blankLine.test(h) && (p += h + `
...
` + I.slice(f);
                  ...
                  if (R2 || d.replace(this.rules.other.tabCharGlobal, "    ").search(this.rules.other.nonSpaceChar) >= 4 || Y.test(d) || ee.test(d) || V.test(d)) break;
                  c += `
` + h;
                }
                R2 = !h.trim(), p += H + `
...
` ? t2[1].slice(0, -1) : t2[1];
      ...
      del(e, t2, n = "") {
        ...
        if (!(r[1] || "") || !n || this.rules.inline.punctuation.exec(n)) {
          ...
          for (p.lastIndex = 0, t2 = t2.slice(-1 * e.length + s); (r = p.exec(t2)) != null; ) {
    ...
    x = class u {
      ...
      constructor(e) {
        ...
        let t2 = { other: m, block: C2.normal, inline: z.normal };
...
` + r.text, this.inlineQueue.pop(), this.inlineQueue.at(-1).src = s.text) : t2.push(r);
      ...
      inlineTokens(e, t2 = []) {
        ...
        for (; e; ) {
          s || (a = ""), s = false;
          let o;
          if (this.options.extensions?.inline?.some((p) => (o = p.call({ lexer: this }, e, t2)) ? (e = e.substring(o.raw.length), t2.push(o), true) : false)) continue;
      ...
      walkTokens(e, t2) {
        ...
        for (let r of e) switch (n = n.concat(t2.call(this, r)), r.type) {
          case "table": {
            ...
            for (let s of i.rows) for (let a of s) n = n.concat(this.walkTokens(a.tokens, t2));
            break;
          }
          case "list": {
            let i = r;
...
function R(e, d) {
  ...
  return {
    ...
    childTokens: ["content"],
    tokenizer(c) {
      e.hasFootnotes || (this.lexer.tokens.push(l), e.tokens = this.lexer.tokens, e.hasFootnotes = true, l.rawItems = [], l.items = []);
      const a = /^\[\^([^\]\n]+)\]:(?:[ \t]+|[\n]*?|$)([^\n]*?(?:\n|$)(?:\n*?[ ]{4,}[^\n]*)*)/.exec(
        c
      );
      if (a) {
...
function A2(e, d = false, l = false) {
  ...
  return {
    ...
    tokenizer(a) {
      const i = /^\[\^([^\]\n]+)\]/.exec(a);
      if (i) {
        const [f, s] = i, n = this.lexer.tokens[0], t2 = n.rawItems.filter(
          (u3) => u3.label === s
        );
        if (!t2.length) return;
        const o = t2[0], r = n.items.filter((u3) => u3.label === s)[0], p = {
          type: "footnoteRef",
          raw: f,
          index: o.refs.length,
          id: "",
    ...
    renderer({ index: a, id: i, label: f }) {
      c = 0;
      const s = encodeURIComponent(f), n = l ? T2(f) : i, t2 = a > 0 ? `-${a + 1}` : "";
      return `<sup><a id="${e}ref-${s}${t2}" href="#${e + s}" data-${e}ref aria-describedby="${e}label">${d ? `[${n}]` : n}</a></sup>`;
    }
  };
}
function E2(e, d, l, c, a, i) {
  return {
    name: "footnotes",
    renderer({ raw: f, items: s = [] }) {
      if (s.length === 0) return "";
      const n = s.reduce(
        (p, { label: u3, content: x2, refs: y2 }) => {
          const b2 = encodeURIComponent(u3), $2 = this.parser.parse(x2).trimEnd(), w2 = $2.endsWith("</p>");
          let h = `<li id="${e + b2}">
`;
          return h += w2 ? $2.replace(/<\/p>$/, "") : $2, y2.forEach((I, F2) => {
            const g2 = i.replace("{0}", u3);
            let m2, k2;
            if (F2 > 0) {
              const L2 = F2 + 1;
              m2 = `\u21A9<sup>${L2}</sup>`, k2 = `-${L2}`;
              ...
              m2 = "\u21A9", k2 = "";
            h += ` <a href="#${e}ref-${b2}${k2}" data-${e}backref aria-label="${g2}">${m2}</a>`;
          }), h += w2 ? `</p>
` : `
`, h += `</li>
`, p + h;
      ...
      let t2 = "";
      l && (t2 += `<hr data-${d}footnotes>
`);
      let o = "";
      c && (o = ` class="${c}"`);
      let r = "";
      return a && (r = ` class="${a}"`), t2 += `<section${o} data-${d}footnotes>
`, t2 += `<h2 id="${e}label"${r}>${f.trimEnd()}</h2>
`, t2 += `<ol>
${n}</ol>
`, t2 += `</section>
`, t2;
...
function C3(e = {}) {
  const {
    ...
    backRefLabel: t2 = "Back to reference {0}"
...
var init_visitor = __esm({
  "src/visitor.js"() {
    ...
    Visitor = class {
      ...
      visit(node2) {
        console.log("[VISIT]", node2);
        const method = this[`visit${node2.type}`];
        if (!method) {
          throw new Error(`No visitor for ${node2.type}`);
        }
        return method.call(this, node2);
      }
      ...
      visitBodyRaw(node2) {
        ...
        const openInline = "\uE000";
        const closeInline = "\uE001";
        const withPlaceholders = node2.segments.map((s) => {
          if (s.type === "AslInline") {
      ...
      visitDocument(node2) {
        console.log("[DOCUMENT]", node2);
        node2.statements.forEach((s) => this.visit(s));
        return this.visit(node2.body);
      }
      visitDocumentRaw(node2) {
        node2.statements.forEach((s) => this.visit(s));
        return this.visitBodyRaw(node2.body);
      }
      ...
      visitGraphemeDef(node2) {
        if (node2.args === null) {
          const val = { type: "Graphemes", val: node2.graphemes.map((g2) => this.visit(g2)) };
          node2.idens.forEach((iden) => this.env.define(iden, val));
        } else {
          node2.idens.forEach((iden) => this.env.define(
            iden,
            { type: "Fn->Graphemes", args: node2.args, body: node2.graphemes }
          ));
        }
      }
      // ExprDef
      ...
      visitExprDef(node2) {
        if (node2.args === null) {
          node2.idens.forEach((iden) => this.env.define(iden, this.visit(node2.expr)));
        } else {
          node2.idens.forEach((iden) => this.env.define(
            iden,
            { type: "Fn->Number", args: node2.args, body: node2.expr }
          ));
        }
      }
      ...
      visitIden(node2) {
        ...
        return { type: "thunk", as: (type) => captureScope.lookup(node2.name, type) };
      }
      //unwrap a thunk, or pass-through non-lazy value
      asType(node2, type) {
        console.log("[AS TYPE]", type, node2);
        if (node2.type === "thunk") {
          const as = node2.as(type);
          console.log("[AS TYPE]", as);
          return as;
        }
        if (node2.type === type) {
          console.log("[AS TYPE]", node2);
          return node2;
        }
        return null;
      }
      ...
      visitBlock(node2) {
        ...
        node2.comms.forEach((c) => this.visit(c));
      ...
      visitCommand(node2) {
        const args = node2.args.map((a) => this.visit(a));
        this.turtle.runCommand(node2.name, args, (n, t2) => this.asType(n, t2), (gs) => this.flattenGraphemes(gs));
      }
      // return node("Prod", { head, tail:tail.map( t => {op:t[1], val:t[3]}) }); 
      visitProd(node2) {
        console.log("[PROD]", node2);
        const head = this.visit(node2.head);
        return {
          type: "Number",
          val: node2.tail.reduce((acc, t2) => {
            const factor = this.visit(t2.val);
            console.log("[PROD REDUCE]", acc, t2, factor);
            if (t2.op == "*")
      ...
      visitUnaryOp(node2) {
        switch (node2.op) {
          case "+":
            return this.asType(this.visit(node2.child), "Number");
          case "-":
            const child = this.visit(node2.child);
            const resolved = this.asType(child, "Number");
            return { type: "Number", val: -resolved.val };
          default:
            throw new Error(`Unknown unary op: ${node2.op}`);
        }
      }
      ...
      visitCall(node2) {
        return {
          ...
          as: (type) => {
            console.log("[AS]", type);
            const fnNode = this.env.lookup(node2.fname.name, `Fn->${type}`);
            const actualArgs = node2.args.map((a) => this.visit(a));
            if (actualArgs.length !== fnNode.args.length) {
              const s = fnNode.args.length == 1 ? "" : "s";
              throw new Error(`Function ${node2.fname} expects ${fnNode.args.length} argument${s}, not ${actualArgs.length}`);
            }
            ...
            if (fnNode.type === "Fn->Graphemes") {
              ...
              val = { type: "Graphemes", val: fnNode.body.map((g2) => this.visit(g2)) };
            } else {
      ...
      visitBody(node2) {
        ...
        const parsed = g.parse(withPlaceholders);
        const placeholder = new RegExp(`${openInline}\\d+${closeInline}`, "g");
      ...
      visitGrapheme(node2) {
        console.log("[GRAPHEME NODE]", node2);
        const result = this.visit(node2);
      ...
      flattenGraphemes(result) {
        ...
        if (graphemesResult?.type === "Graphemes") {
          ...
          return graphemesResult.val.flatMap((v2) => this.flattenGraphemes(v2));
        }
        if (graphemesResult?.type === "thunk") {
          console.log("[FLATTEN THUNK]", graphemesResult);
          return this.asType(graphemesResult, "Graphemes").val.flatMap((v2) => this.flattenGraphemes(v2));
        }
      ...
      // = "@@@" _ rows:BlockRows _ "@@@"
      visitAslBlock(node2) {
        const rows = node2.rows.map((row) => {
          if (row === null) {
            return `<div style="height:1em;"></div>`;
          }
          const glyphs = row.flatMap((g2) => this.visitGrapheme(g2));
          return `<div style="display:flex;flex-wrap:wrap;align-items:baseline;">${glyphs.map((g2) => g2.fullHTML).join("")}</div>`;
        });
        return `<div style="display:block;">${rows.join("")}</div>`;
      }
      // "@" graphemes:Graphemes "@"
      visitAslInline(node2) {
        console.log("[ASL INLINE]", node2);
        let glyphs = node2.graphemes.flatMap((g2) => this.visitGrapheme(g2));
        console.log("[ASL INLINE]", glyphs);
        return `<span style="display:inline-flex;flex-wrap:wrap;align-items:baseline;">${glyphs.map((g2) => g2.fullHTML).join("")}</span>`;
      }
      // Row = head:Iden tail:(SPACE IDEN)* { return node("Row", { cols: flatten(head, tail) }); }
      visitRow(node2) {
        return node2.cols.flatMap((c) => this.visitGrapheme(c));
      }
      // chars:MarkdownChar+ { return node("MarkdownText", { text: chars.join('') }) }
      visitMarkdownText(node2) {
        return node2.text;
      }
    };
  }
...
// dist/libs/handshapes.asl
var handshapes_default;
var init_handshapes = __esm({
  "dist/libs/handshapes.asl"() {
...
// dist/libs/alphabet.asl
var alphabet_default;
var init_alphabet = __esm({
  "dist/libs/alphabet.asl"() {
...
// dist/libs/diacritics.asl
var diacritics_default;
var init_diacritics = __esm({
  "dist/libs/diacritics.asl"() {
...
var require_main = __commonJS({
  "src/main.js"() {
    ...
    init_layouts();
    var tabContents = {
      readme: { edit_sx: 0, edit_sy: 0, out_sx: 0, out_sy: 0, content: README_default },
      vocab: { edit_sx: 0, edit_sy: 0, out_sx: 0, out_sy: 0, content: vocab_default },
      diacritics: { edit_sx: 0, edit_sy: 0, out_sx: 0, out_sy: 0, content: diacritics_default },
      handshapes: { edit_sx: 0, edit_sy: 0, out_sx: 0, out_sy: 0, content: handshapes_default },
      arrows: { edit_sx: 0, edit_sy: 0, out_sx: 0, out_sy: 0, content: arrows_default },
      layouts: { edit_sx: 0, edit_sy: 0, out_sx: 0, out_sy: 0, content: layouts_default },
      alphabet: { edit_sx: 0, edit_sy: 0, out_sx: 0, out_sy: 0, content: alphabet_default },
      scratch: { edit_sx: 0, edit_sy: 0, out_sx: 0, out_sy: 0, content: "import vocab\nimport alphabet\n===\n# Example document\n\nTry playing around!" }
    };
    var activeTab = "readme";
    function switchTab(tabName) {
      ...
      document.querySelectorAll(".tab").forEach((t2) => {
        t2.classList.toggle("active", t2.dataset.tab === tabName);
      });
      execute();
    }
    document.querySelectorAll(".tab").forEach((t2) => {
      t2.addEventListener("click", () => switchTab(t2.dataset.tab));
    });
    ...
    async function resolveImports(ast, tabContents2) {
      const resolvedStatements = [];
      for (const stmt of ast.statements) {
        if (stmt.type === "Import") {
          try {
            const tabKey = Object.keys(tabContents2).find(
              (k2) => k2 === stmt.path || k2 === stmt.path.replace(".asl", "")
            );
            const source = tabKey ? tabContents2[tabKey].content : await fetch("libs/" + stmt.path + ".asl").then((r) => {
              if (!r.ok) throw new Error(`Could not import "${stmt.path}.asl": ${r.status} ${r.statusText}`);
              return r.text();
            });
            const importedAst = peg$parse(source);
            await resolveImports(importedAst, tabContents2);
      ...
      try {
        ...
        switch (chosenFormat) {
          ...
          case "HTML":
            ...
            const borderStyle = showBorders ? `svg { border: 2px dashed lightgrey; }` : `svg { border: none; }`;
        ...
        if (e.location) {
          ...
          out2.srcdoc = `<!DOCTYPE html><html><body><div style="color:#911c1c;font-family:monospace;padding:12px;">${msg}</div></body></html>`;
          highlightError(editor, start, end);
        } else {
          out2.srcdoc = `<!DOCTYPE html><html><body><pre style="color:#911c1c;font-family:monospace;padding:12px;">${e.message}
${e.stack}</pre></body></html>`;
    ...
    document.getElementById("svg-increase").addEventListener("click", () => {
      if (svgSizeIndex < sizes.length - 1) {
        svgSizeIndex++;
    ...
    document.getElementById("font-increase").addEventListener("click", () => {
      if (fontSizeIndex < sizes.length - 1) {
        fontSizeIndex++;
